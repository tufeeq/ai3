...
const provenanceYml=fs.readFileSync('.github/workflows/validation-provenance.yml','utf8');
...
assert(/workflow_run:\s*\n\s*workflows:\s*\["TAGX3 Validation Snapshots"\]/m.test(provenanceYml),'capture provenance must refresh after successful snapshot workflows instead of depending only on GitHub schedule delivery');
assert(/github\.event_name != 'workflow_run' \|\| github\.event\.workflow_run\.conclusion == 'success'/.test(provenanceYml),'failed snapshot runs must never refresh provenance');
assert(/npm run check/.test(provenanceYml),'provenance publication must remain behind syntax/static contracts');
assert(/node tests\/validation-capture-provenance-contract\.mjs/.test(provenanceYml),'provenance publication must run its targeted contract');
assert(/npm run validation:capture-provenance/.test(provenanceYml),'event-driven provenance rebuild must remain present');
assert(!/npm run validation:scorecard/.test(provenanceYml),'lightweight provenance refresh must not invoke the heavy causal scorecard');
assert(/group:\s*tagx3-validation-provenance/.test(provenanceYml),'provenance refresh must use an independent concurrency lane');
