(function(root,factory){
  ...
  function evaluateBase(base,snapshots,protocol){
    ...
    for(const [symbol,o] of obs){
      ...
      for(const h of horizons){
        ...
        outcomes[h]={status:'OK',snapshotAt:target.capturedAt,returnPct:pct(p0,p1),mfePct:path.mfePct,maePct:path.maePct,pathObservationCount:path.count,peakAt:path.peakAt,troughAt:path.troughAt,baseSource,targetSource,sourceChanged:baseSource!==targetSource,baseLiveBacked,targetLiveBacked,liveBackedChanged:baseLiveBacked!==targetLiveBacked};
      }
  ...
  function metricsForHorizon(rows,h,protocol){
    ...
    const singlePointPath=valid.filter(r=>r.outcomes[h]?.pathObservationCount===1);
    ...
    return {horizonMin:+h,validCount:valid.length,moverCount:movers.length,detectedCount:detected.length,actionableCount:actionable.length,flatOutcomeCount:flat.length,flatOutcomeRate:valid.length?flat.length/valid.length:null,singlePointPathCount:singlePointPath.length,singlePointPathRate:valid.length?singlePointPath.length/valid.length:null,sourceChangedOutcomeCount:sourceChanged.length,sourceChangedOutcomeRate:valid.length?sourceChanged.length/valid.length:null,liveBackedChangedOutcomeCount:liveBackedChanged.length,liveBackedChangedOutcomeRate:valid.length?liveBackedChanged.length/valid.length:null,earlyCaptureRate:movers.length?captured.length/movers.length:null,missedMoverRate:movers.length?missed.length/movers.length:null,falsePositiveRate:detected.length?falsePos.length/detected.length:null,actionableFalsePositiveRate:actionable.length?actionableFalsePos.length/actionable.length:null,avgDetectedMFE:avg(detected,'mfePct'),avgDetectedMAE:avg(detected,'maePct'),avgActionableMFE:avg(actionable,'mfePct'),avgActionableMAE:avg(actionable,'maePct'),excludedOutcomeCounts:excluded};
  }
