...
function secRecent(sub,symbol,title){
  ...
  for(let i=0;i<forms.length;i++){
    ...
    out.push({symbol,title,type:'SEC',form:forms[i],publishedAt:accepted[i]||dates[i],headline:`${forms[i]} filing — ${title}`,url,source:'SEC EDGAR'});
  }
  return out.slice(0,8);
}

async function gdeltNews(title,symbol){
  if(!title)return[];
  const q=encodeURIComponent(`\"${title.replace(/\b(Inc|Corp|Corporation|Ltd|Limited|PLC|Holdings?)\.?\b/gi,'').trim()}\"`);
  const url=`https://api.gdeltproject.org/api/v2/doc/doc?query=${q}&mode=ArtList&maxrecords=6&format=json&timespan=3d&sort=datedesc`;
  try{
    const j=await getJson(url,{headers:{'User-Agent':UA},timeout:12000});
    return (j.articles||[]).slice(0,6).map(a=>({symbol,title,type:'NEWS',headline:a.title||'News',publishedAt:a.seendate||null,url:a.url||null,domain:a.domain||null,source:a.domain||'GDELT source',language:a.language||null}));
  }catch{return[]}
}
...
async function main(){
  ...
  for(const p of picks){
    const m=map.get(p.symbol); if(!m)continue;
  ...
  for(const p of picks.slice(0,20)){
    const c=companies[p.symbol]; if(!c)continue;
    events.push(...await gdeltNews(c.title,p.symbol));
    await sleep(180);
  }
  ...
  const payload={schemaVersion:1,generatedAt:iso,sourceStatus:{live:!!live,secMapping:!!mapping},shortlist:picks,companies,eventCount:dedup.length,events:dedup,errors:errors.slice(0,30),policy:'Public-source intelligence. SEC filings are primary-source events; GDELT is discovery-only and requires source-level verification before trade decisions.'};
  await fs.mkdir('data',{recursive:true}); await fs.writeFile(OUT,JSON.stringify(payload,null,2));
  console.log(`wrote ${OUT}: ${dedup.length} events for ${Object.keys(companies).length} mapped companies`);
}
